...
def index():
    ...
    print(bed_changes)  # Check if this returns the correct list of people and changes

    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT SUM(rails), SUM(iphones), SUM(logos), SUM(knobs), SUM(ipadbase), SUM(snapfits) FROM production")
    totals = c.fetchone()
    print(bed_changes)  # Debugging statement
...
def submit_master():
    ...
    # Insert the master summary and bed changes
    c.execute('''
    ...
    # Assuming you want to save the bed changes for each person per cycle, you need a separate table.
